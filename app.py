...
for i, message in enumerate(st.session_state.messages):
    ...
    with st.chat_message(message["role"], avatar=avatar_icon):
        ...
        if message["role"] == "assistant":
            ...
            st.markdown(f"""<div class="action-buttons-container"><button class="btn-action btn-blue" onclick="window.print()">🖨️ Cetak / PDF</button><a class="btn-action btn-green" href="data:text/html;base64,{b64_html_hist}" download="Dokumen_SeHe_AI_{i}.html">💾 Simpan .HTML</a></div>""", unsafe_allow_html=True)
